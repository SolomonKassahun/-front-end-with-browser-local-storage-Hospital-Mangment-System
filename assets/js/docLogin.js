...
document.addEventListener("DOMContentLoaded", () => {
  ...
  patientDB.onerror = function(e){
    ...
    patientDB.onsuccess = function(e){
        ...
        let ob = DB.transaction(['doctor'],"readwrite").objectStore("doctor")
        ob.add(doc1)
  ...
  function addNewTask(e) {
    e.preventDefault();
    let objectStore = DB.transaction('doctor').objectStore('doctor')
    objectStore.openCursor().onsuccess = function(e){
        ...
        if(cursor){
            ...
            if((cursor.value.fullName == userN.value) && (cursor.value.password == pass.value)){
              location.href = `t1.html?dname=${cursor.value.fullName}`;
              form.reset();
